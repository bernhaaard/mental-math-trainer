...
**Objective**: Implement the study mode with interactive content for each calculation method, including mathematical foundations, examples, and self-practice exercises.

**Exit Criteria**: All six methods have complete study content; interactive examples work correctly; progressive curriculum guides learning.

---

## Overview

Study Mode provides educational content at three levels:
1. **Introduction**: What the method is and when to use it
2. **Mathematical Foundation**: Why it works (algebraic proofs)
3. **Deep Dive**: Advanced theory (ring theory, geometric interpretations)

---

## Study Content Structure

### `src/lib/types/study.ts`

```typescript
import type { MethodName, Solution } from './index';
...
export interface StudyProgress {
  method: MethodName;
  introductionRead: boolean;
  foundationRead: boolean;
...
### `src/lib/core/study-content-generator.ts`

```typescript
import type { StudyContent, StudyExample, InteractiveExercise, MethodName } from '@/lib/types';
...
The distributive property was used implicitly in ancient Babylonian mathematics (circa 2000 BCE) for multiplication. The algebraic formalization came much later with the development of abstract algebra in the 19th century.
    ...
    prerequisites: [],
    nextMethods: ['near-power-10', 'factorization']
  },

  'difference-squares': {
    method: 'difference-squares',
    introduction: `
...
This works best when μ is round (10, 50, 100) and d is small.
    ...
    prerequisites: ['distributive'],
    nextMethods: ['squaring']
  },

  'near-power-10': {
    method: 'near-power-10',
    introduction: `
...
- |offset| ≤ 15 (small enough for easy multiplication)
...
This generalizes to any power, though mental math typically uses only n=2.
    ...
    prerequisites: ['distributive', 'difference-squares'],
    nextMethods: []
  },
...
This technique is popularized in Vedic mathematics as "nikhilam" (subtraction from the base). While attributed to ancient Indian texts, its modern formulation is from the 20th century.
    ...
    prerequisites: ['distributive', 'near-power-10'],
    nextMethods: []
  },
...
export function generateStudyContent(method: MethodName): StudyContent {
  const base = STUDY_CONTENT[method];
...
function generateExamples(method: MethodName): StudyExample[] {
  const examplePairs: Record<MethodName, Array<[number, number]>> = {
    'distributive': [[47, 89], [23, 67], [34, 56]],
    'difference-squares': [[47, 53], [96, 104], [43, 57]],
    'near-power-10': [[98, 47], [102, 35], [997, 23]],
    'squaring': [[73, 73], [25, 25], [67, 69]],
    'near-100': [[97, 94], [103, 98], [88, 96]],
    'factorization': [[25, 48], [35, 24], [125, 56]]
  };

  return examplePairs[method].map(([num1, num2]) => {
    const ranking = selectOptimalMethod(num1, num2);
    return {
      num1,
      num2,
      solution: ranking.optimal.solution,
      pedagogicalNotes: [`This demonstrates the ${method} technique.`],
      commonMistakes: ['Remember to check your arithmetic at each step.']
    };
  });
}

function generateExercises(method: MethodName): InteractiveExercise[] {
  const exercisePairs: Record<MethodName, Array<{ nums: [number, number]; difficulty: 'easy' | 'medium' | 'hard' }>> = {
    'distributive': [
  ...
  return exercisePairs[method].map((ex, i) => ({
    id: `${method}-exercise-${i}`,
    num1: ex.nums[0],
    num2: ex.nums[1],
...
function generateHints(method: MethodName, num1: number, num2: number): string[] {
  switch (method) {
    case 'difference-squares':
      const mid = (num1 + num2) / 2;
      const dev = Math.abs(num2 - num1) / 2;
      return [
        `What is the midpoint of ${num1} and ${num2}?`,
        `The midpoint is ${mid}. How far is each number from the midpoint?`,
        `Apply: ${mid}² - ${dev}² = ?`
      ];
    case 'near-100':
      return [
        `How far is each number from 100?`,
        `Calculate (100 - deficit1 - deficit2) × 100`,
        `Add the product of the deficits`
      ];
    default:
      return [
...
- [ ] All six methods have complete study content
- [ ] Mathematical proofs are correct and clear
- [ ] Examples generate correct solutions
- [ ] Interactive exercises have appropriate difficulty progression
- [ ] Hints guide without giving away the answer
- [ ] Prerequisites create logical learning path
...
- Content requirements: `docs/PROJECT_REQUIREMENTS.md` Section 4.4
